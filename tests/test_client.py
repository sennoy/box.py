...
class TestClient(unittest.TestCase):
    ...
    def test_download_file_partially(self):
        client = self.make_client("get", "files/123/content", params={}, headers={'Range': 'bytes=0-11'}, result='hello world', stream=True)
        response = client.download_file(123, bot_range=0, top_range=11)
        self.assertEqual('hello world', response.text)

    def test_download_file_partially_with_version(self):
        client = self.make_client("get", "files/123/content", params={'version': 1000}, headers={'Range': 'bytes=0-11'}, result='hello world', stream=True)
        response = client.download_file(123, bot_range=0, top_range=11, version=1000)
        self.assertEqual('hello world', response.text)
